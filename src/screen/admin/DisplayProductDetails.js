...
export default function DisplayAllProductDetails() {
  ...
  const FetchAllSubCategory = async (cid) => {
    var result = await postData("category/Fetch_all_subcategory_by_category", {
      categoryid: cid,
    });
  ...
  const FetchAllProduct = async (bid) => {
    var result = await postData("brand/Fetch_all_Product_by_Brand", {
      brandid: bid,
    });
  ...
  const handleEdit = async () => {
    ...
    if (submit) {
      ...
      var result = await postData(
        "productdetails/update_product_DetailData",
        body
      );
      if (result.status) {
        Swal.fire({
  ...
  const handlePictureEdit = async () => {
    ...
    var result = await postData(
      "productdetails/edit_productdetail_picture",
      formData
    );
    if (result.message) {
      Swal.fire({
  ...
  const handleDelete = async (rowData) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        var result = await postData(
          "productdetails/delete_productdetails_data",
          body
        );
        if (result.status) {
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={6}>
                {showBtn ? (
                  ...
                  >
                    <Button variant="contained" onClick={handlePictureEdit}>
                      Save
                    </Button>
                    <Button variant="contained" onClick={handleCancelPicture}>
                      Cancel
                    </Button>
                  </div>
                ) : (
    ...
    return (
      <MaterialTable
        ...
        columns={[
          ...
          {
            title: "Brand/Product/Weight/Weight Type",
            render: (rowData) => (
              <div>
                <div>{rowData.brandname}</div>
                <div>{rowData.productname} </div>
                <div>{rowData.weight} </div>
                <div>{rowData.weighttype}</div>
              </div>
            ),
          },

          {
            title: "Product Subname/Concern Name",
            render: (rowData) => (
              <div>
                <div>{rowData.productsubname}</div>
                <div>{rowData.concernname}</div>
              </div>
            ),
          },
  ...
  return (
    <div className={classes.root}>
      <div className={classes.tablebox}>{ShowProduct()}</div>
