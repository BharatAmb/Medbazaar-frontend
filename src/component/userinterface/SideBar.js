...
export default function SideBar() {
  var category = [
    {
      ...
      categoryname:
        "Home Care,Elderly Care ,Mom and Baby,Personal Care,Food and Beverage,Pet Supplies",
      picture: "19.webp",
    },
  ];
  var brand = [
    {
      brandid: 1,
      brandname: "Cadbury,Novology,Sunfeast,Zandu",
      picture: "1.webp",
    },
  ];
  ...
  const categoryName = (item) => {
    ...
    return name.map((name) => <div>{name}</div>);
  };

  const brandName = (item) => {
    const name = item.brandname.split(",");
    return name.map((name, index) => <div>{name}</div>);
  };
      ...
      return (
        <div>
          ...
          >
            <Grid item xs={6}>
              <Divider
                style={{ borderWidth: 1.8, display: "flex", width: "auto" }}
              ></Divider>
              <div
                style={{
                  fontSize: 18,
