...
export default function PaymentDetails(props) {
  ...
  const options = {
    ...
    image: `${serverURL}/images/medbazzar-logo.png`,
  ...
  const handleLogin = () => {
    if (caption.toUpperCase() === "MAKE PAYMENT") openPayModal();
    ...
    return (
      <div style={{ margin: 20 }}>
        <Grid container spacing={1}>
          ...
          <Grid item xs={12}>
            <div>Price may very depending on te product batch</div>
          </Grid>
        </Grid>
      </div>
    );
  ...
  const PaperBox2 = () => {
    return (
      <Paper elevation={0} style={{ paddingLeft: 20 }}>
        <Grid container spacing={1}>
          <Grid item xs={12}>
            <div>Use Coopen</div>
            <div>Also get a gift code agter pacing this order</div>
          </Grid>
        </Grid>
      </Paper>
    );
  ...
  return (
    <div style={{ fontFamily: "kanit", margin: 20 }}>
      <Grid
        container
        spacing={2}
        style={{ marginTop: 17, boxShadow: "inset 1px 1px 1px 1px " }}
      >
        <Grid item xs={12}>
          {PaperBox1()}
        </Grid>
